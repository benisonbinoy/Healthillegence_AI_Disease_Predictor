...
import { ArrowLeft, Bug, TrendingUp, AlertCircle, Upload, X, Eye } from 'lucide-react';
...
export default function MalariaPredictionPage() {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/predict/malaria', { method: 'POST', body: formData });
      const result = await response.json();
      if (!result.success) {
        setError(result.error || 'Prediction failed. Ensure the Python backend is running.');
        return;
      }
      setPrediction(result);
      if (result.gradcam_image) { setGradcamImage(result.gradcam_image); setShowGradcam(true); }
    } catch (err: any) {
      setError('Cannot connect to prediction server. Make sure the Python backend is running on port 5000.');
  ...
  const handleReset = () => {
    ...
    setGradcamImage(null); setShowGradcam(false); setError(null);
  };

  if (!isAuthenticated) return null;
  const isParasitized = prediction?.prediction === 'Parasitized';
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Malaria Detection</h1>
                <p className="text-gray-600 dark:text-gray-400">Upload a blood-cell image for AI-powered analysis</p>
              </div>
            </div>
          </motion.div>
          ...
          {!prediction ? (
              ...
              {!selectedImage ? (
                <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-all duration-300 ${isDragActive ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-gray-300 dark:border-gray-700 hover:border-primary-500 dark:hover:border-primary-500'}`}>
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                    {isDragActive ? 'Drop the image here' : 'Upload Cell Image'}
                  </h3>
                ...
                <div className="space-y-4">
                  <div className="relative rounded-xl overflow-hidden border-2 border-gray-200 dark:border-gray-800">
                    <div className="relative w-full h-64">
                      {imagePreview && <Image src={imagePreview} alt="Selected cell image" fill className="object-contain" />}
                    </div>
                  ...
                  <button onClick={handlePredict} disabled={loading}
                    ...
                    {loading ? (<><div className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-white" /> Analysing Image&hellip;</>) : (<><Bug className="w-5 h-5" /> Detect Malaria</>)}
                  </button>
                  {error && (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                      <Eye className="w-5 h-5 text-green-500" /> Visual Analysis
                    </h3>
                    {gradcamImage && (
                  ...
                  <div className={`grid gap-4 ${gradcamImage ? 'grid-cols-2' : 'grid-cols-1'}`}>
                    ...
                    <div className="space-y-2">
                      <p className="text-xs text-center font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Original Image</p>
                      <div className="relative w-full h-52 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                        <Image src={imagePreview} alt="Original cell image" fill className="object-contain" />
                      </div>
                    </div>
                    {/* Grad-CAM */}
                    {gradcamImage && (
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-center text-gray-400 dark:text-gray-500">Red regions  highest model attention</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Result card */}
              <div className={`p-8 rounded-2xl border-2 ${isParasitized ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800'}`}>
                <div className="flex items-center gap-4 mb-6">
                  <AlertCircle className={`w-12 h-12 ${isParasitized ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`} />
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Detection Result</h2>
                    <p className={`text-xl font-bold ${isParasitized ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>{prediction.prediction}</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="bg-white dark:bg-gray-900 rounded-xl p-4 border border-gray-200 dark:border-gray-800">
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Uninfected Probability</div>
                    <div className="text-2xl font-bold text-green-600 dark:text-green-400">{prediction.probability?.uninfected?.toFixed(1)}%</div>
                  </div>
                  <div className="bg-white dark:bg-gray-900 rounded-xl p-4 border border-gray-200 dark:border-gray-800">
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Parasitized Probability</div>
                    <div className="text-2xl font-bold text-red-600 dark:text-red-400">{prediction.probability?.parasitized?.toFixed(1)}%</div>
                  </div>
                </div>
              </div>
